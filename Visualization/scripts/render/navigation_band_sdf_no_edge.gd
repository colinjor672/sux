# NavigationBandSDFNoEdge.gd
# SDF 屏幕空间距离场渲染 —— 无边界细线版本
# 彻底消除三角形拼接导致的几何重叠与颜色叠加
# 每像素精确计算到中心线的距离，无 UV 插值失真
# 4 层合成（glow+body+center+flow），无左右边界 Line2D
class_name NavigationBandSDFNoEdge
extends Control
...
@export var glow_extra_width := 150.0
...
func _ready() -> void:
	...
	_init_sdf_mesh()
	print("[NavBandSDFNoEdge] 就绪: SDF 距离场渲染（无边界细线）, MAX_SEGMENTS=%d" % MAX_SEGMENTS)
